...
function normalizeSettings(payload: unknown): SettingsShape | null {
  ...
  const quietSrc = isObject(src.quietDown) ? src.quietDown : null;
  ...
  return {
    ...
    quietDown: quietSrc
      ? {
          enabled: quietSrc.enabled === true,
          strategy: quietSrc.strategy === "mute" ? "mute" : "lower",
          targetVolumePct: Math.min(80, Math.max(5, asNumber(quietSrc.targetVolumePct) ?? 20)),
          restoreAfter: quietSrc.restoreAfter !== false,
          includeFireTv: quietSrc.includeFireTv === true,
          note: asString(quietSrc.note) ?? null,
        }
      : undefined,
  ...
  useEffect(() => {
    async function loadToday() {
      ...
      try {
        ...
        const res = await apiFetchWithAmazonRepair("/api/prayer-times/today");
        if (!res.ok) {
          if (res.status === 401) {
            throw new Error("Your Amazon session expired. Please reconnect Amazon.");
          }
  ...
  useEffect(() => {
    ...
    async function loadHadith() {
      ...
      try {
        ...
        const res = await apiFetchWithAmazonRepair(
        ...
        <div className="mb-8 space-y-4">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              {userSettings?.quietDown?.enabled && (
                <p className="text-slate-500 text-xs mt-1">
                  Quiet-down policy: {userSettings.quietDown.strategy === "mute" ? "Mute selected devices" : `Lower selected devices to about ${userSettings.quietDown.targetVolumePct}%`}
                  {userSettings.quietDown.includeFireTv ? " · Fire TV included" : " · Fire TV excluded"}
                </p>
              )}
